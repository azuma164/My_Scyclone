...
class Scyclone(pl.LightningModule):
    ...
    def __init__(self, sampling_rate: int, noiseless_D: bool = False):
        ...
        self.griffinLim = GriffinLim(n_fft=254, n_iter=1024)
        #256
    ...
    def test_step(self, batch: Tensor, batch_idx: int):
        ...
        # fake_A = self.G_B2A(real_B)
        # ReLU for preventing minus value for GriffinLim
        print("fake_B: {}".format(fake_B))
        print("fake_B_size: {}".format(fake_B.size()))
        fake_B_wave = self.griffinLim(F.relu(fake_B))
        print("fake_B_wave: {}".format(fake_B_wave))
        print("fake_B_size: {}".format(fake_B_wave.size()))
        # fake_A = self.griffinLim(F.relu(fake_A))
        # fake_B_wave_path = "scyclonepytorch/data/dataset_fake_obamawave"
        # fake_A_wave_path = "scyclonepytorch/data/dataset_fake_apiwave"
        # save(fake_b_wave_path, fake_B_wave, self.hparams["sampling_rate"])
        # save(fake_A_wave_path, fake_A_wave, self.hparams["sampling_rage"])
        print(fake_B_wave.size())

        return {
            "wave": {"Validation/A2B": fake_B_wave}
        }
    
    def test_epoch_end(self, outs):
        fake_B_wave = outs[0]["wave"]["Validation/A2B"]
        print(outs)
        # fake_A = out["wave"]["Validation/B2A"]
        print(fake_B_wave.size())
        print("outsize is {}".format(len(outs)))      
        ...
        print("fake_B size is {}".format(fake_B_wave.size()))
        save(fake_B_wave_path, fake_B_wave, self.hparams["sampling_rate"])
        # save(fake_A_wave_path, fake_A, self.hparams["sampling_rate"])
        return
