...
def test(args: Namespace, datamodule: LightningDataModule) -> None:
    ...
    print("you use gpu if {} is 1".format(gpus))
    model = Scyclone(args.sampling_rate, args.noiseless_d)
    ckpt_cb = ModelCheckpoint(every_n_epochs=60, save_last=True, save_top_k=1, monitor="val_loss")
    trainer = pl.Trainer(
        gpus=1,
        ...
        #ckpt_path = "scyclonepytorch/data/logs/default/version_-1/checkpoint/last.ckpt"
    ...
    # training
    trainer.test(model = model, datamodule=datamodule, ckpt_path = "scyclonepytorch/logs/default/version_-1/checkpoints/epoch=7999-step=495999.ckpt")
